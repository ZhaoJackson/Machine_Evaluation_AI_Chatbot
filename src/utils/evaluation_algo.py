...
def evaluate_ethical_alignment(hypothesis):
    ...
    ethical_score = 1 / (1 + np.exp(-scores[1]))
...
def evaluate_sentiment_distribution(hypothesis):
    ...
    balanced_score = sum(emotion_scores.values()) / len(RELEVANT_EMOTIONS)
    max_emotion = max(emotion_scores.values(), default=0)
    min_emotion = min(emotion_scores.values(), default=0)
    if abs(max_emotion - min_emotion) > SENTIMENT_BALANCE_PENALTY:
        balanced_score *= 0.95
    return round(balanced_score, 2)
...
def evaluate_inclusivity_score(hypothesis):
    ...
    return round(inclusive_count / len(words), 2) if len(words) > 0 else 0
...
def evaluate_complexity_score(hypothesis):
    ...
    complexity_score = max(
        ...
        (COMPLEXITY_AVG_SENTENCE_LENGTH_WEIGHT * avg_sentence_length +
         COMPLEXITY_FK_SCORE_WEIGHT * fk_score) / 2
    )
